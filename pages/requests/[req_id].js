...
<<<<<<< HEAD
    ...
    const renderAcceptanceForDoctors = () => {
        return request?.acceptingDoctorUid ? (
                ...
                <p>{request.acceptingDoctorUid === user.uid ? (
                    "You have accepted this request."
                ): ( "Another doctor has accepted this request." )}</p>
    ...
    const renderAcceptanceForPatient = () => {
        return request?.acceptingDoctorUid ? (
            <>
                ...
                <DoctorRow details={acceptingDoctor}/>
                <p>Message from the doctor: </p>
                <Textarea value={request?.acceptingDoctorResponse} readOnly resize={"none"}/>
            </>
        ) : (
            <p>Waiting for a doctor to accept this request.</p>
        );
    ...
    const renderFulfillment = (fulfilled) => {
        ...
        if (fulfilled) {
            return (
                <Box
                    ...
                    p={8}
                    width={"full"}
                ><p>This request was fulfilled.</p></Box>
            )
        }
            ...
            <Stack spacing={8} mx={'auto'} width={"full"} padding={"50px"} py={12}>
                ...
                >
                    <Stack spacing={4}>
                        ...
                        <p>requested by <b>{patient?.firstName} {patient?.lastName}</b> @{patient?.username}</p>
                        <Heading style={{marginTop:"30px"}} fontSize={'2xl'}>Message</Heading>
                        <Textarea readOnly rows={10} resize={"none"} value={request?.message} />
                        {(patient?.address) && (
                            <>
                            <p>{patient?.firstName} {patient?.lastName} lives at <b>{patient?.address}</b>.</p>
                            <iframe
                                height="450"
                                loading="lazy"
                                allowfullscreen
                ...
                {user?.userType === "Patient" && request?.acceptingDoctorUid && renderFulfillment(request?.fulfilled )}
            </Stack>
        </Flex>
=======
    fetchData()
...
>>>>>>> 88592a1a975716a06a8d1ce271f82f0b3f89792c
    )
  }
